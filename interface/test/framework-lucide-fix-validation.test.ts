...
describe('Framework Lucide Import Consolidation - Validation Test', () => {
  ...
  // Mock the image URL fix logic (updated to match framework)
  function fixMalformedUrls(code: string): string {
    ...
    code = code.replace(/src:\s*""([^"]+)""/g, `src: "$1"`);
    code = code.replace(/image:\s*""([^"]+)""/g, `image: "$1"`);
    code = code.replace(/avatar:\s*""([^"]+)""/g, `avatar: "$1"`);
    code = code.replace(/poster:\s*""([^"]+)""/g, `poster: "$1"`);
