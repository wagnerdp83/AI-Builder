...
export async function executeComponentDelete(
  ...
  try {
    ...
    const componentRegex = new RegExp(
      `(<SectionWrapper(?:\\s+[^>]*?)id="${component.toLowerCase()}"(?:\\s+[^>]*?)>\\s*<${componentName}\\s*/>\\s*<\\/SectionWrapper>)`,
      's'
    );
    ...
    if (pageContent === newPageContent) {
      // Fallback regex for slightly different structure
      const fallbackRegex = new RegExp(`(<SectionWrapper(?:\\s+[^>]*?)id="${component.toLowerCase()}"(?:\\s+[^>]*?)>[\\s\\S]*?<\\/${componentName}>[\\s\\S]*?<\\/SectionWrapper>)`, 's');
      newPageContent = pageContent.replace(fallbackRegex, '');
    }
